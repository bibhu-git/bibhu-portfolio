...
const Navbar = () => {
  ...
  return (
    <nav className="shadow-lg bg-black/30 backdrop-blur-md border-b border-gray-700 fixed top-0 w-full z-50">
          ...
          >
            {links.map((link) => (
              <Link
                ...
                className="text-gray-200 font-semibold hover:text-indigo-400 transition"
              >
                {link.name}
              </Link>
            ))}
      ...
      <div
        className={`md:hidden bg-black/30 backdrop-blur-md shadow-md overflow-hidden transition-[max-height] duration-300 ease-in-out ${
